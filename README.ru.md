...
Для каждого симлинка и шаблона OnlineConf отслеживает, от каких параметров он зависит — все узлы, через которые проходит его разрешение, и узел, на который он в итоге указывает. Это учитывает симлинки и шаблоны внутри кейсов (на любом уровне вложенности), а также цепочки симлинков, ссылающихся друг на друга. Зависимости вычисляются один раз при создании, изменении или перемещении симлинка/шаблона, поэтому описанные ниже проверки сводятся к единственному индексированному запросу и не вносят заметной задержки даже на сложных деревьях.

Две операции над параметром, от которого всё ещё зависят другие симлинки/шаблоны, запрещаются с ошибкой, перечисляющей ссылающиеся симлинки/шаблоны:

* **Удаление** — удаление параметра оставило бы эти симлинки/шаблоны висячими.
* **Перемещение без оставления симлинка** — при перемещении параметра (или поддерева, содержащего параметр, на который ссылаются) старые пути перестают разрешаться, что ломает ссылающиеся объекты так же, как и удаление. Перемещение *с* оставлением симлинка по старому пути разрешено: симлинк сохраняет разрешимость старых путей, а само перемещение и создание симлинка выполняются атомарно.

Симлинки, указывающие на одну из своих родительских папок (в том числе на корень), обрабатываются корректно и никогда не блокируют собственное удаление. Цепочки зависимостей прослеживаются до настраиваемой глубины (`maxSymlinkDepth`, по умолчанию 10); параметр, на который ссылаются только через цепочку длиннее этой глубины, не защищён. Глубина задаётся параметром `/onlineconf/deleted-key-symlinks-check-depth` (целое число в текстовом виде); её увеличение перестраивает информацию о зависимостях в фоне и применяется без перезапуска сервера.

Для полного отключения этих проверок следует установить `/onlineconf/disable-deleted-key-symlinks-check` в любое непустое текстовое значение, отличное от `0`.
