...
Симлинки, указывающие на одну из своих родительских папок (в том числе на корень), обрабатываются корректно и никогда не блокируют собственное удаление. Цепочки зависимостей прослеживаются до настраиваемой глубины (по умолчанию 10); параметр, на который ссылаются только через цепочку длиннее этой глубины, не защищён. Глубина задаётся параметром `/onlineconf/deleted-key-symlinks-check-depth` (целое число в текстовом виде); её увеличение перестраивает информацию о зависимостях в фоне и применяется без перезапуска сервера.
